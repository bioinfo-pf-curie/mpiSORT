...
void ParallelBitonicSort(int my_rank, int dimension,
	...
	*/
    ...
    for (proc_set_size = 2, and_bit = 2; proc_set_size <= dimension;
        ...
        if ((my_rank & and_bit) == 0){
            ...
            Par_bitonic_sort_incr(list_size, local_list, local_index, proc_set_size, MPI_COMM_WORLD);
            //fprintf(stderr, "Rank %d :::::[BITONIC SORT] after Par_bitonic_sort_incr  \n", my_rank);
        }
        else{

            Par_bitonic_sort_decr(list_size, local_list, local_index, proc_set_size, MPI_COMM_WORLD);
...
void Par_bitonic_sort_incr(
    ...
    for (stage = 0; stage < proc_set_dim; stage++) {
        ...
        if (my_rank < partner){
            Merge_split(list_size, local_list, local_index, LOW, partner, comm, my_rank);
        }
        else{
            Merge_split(list_size, local_list, local_index, HIGH, partner, comm, my_rank);
        }
        eor_bit = eor_bit >> 1;
    }
