...
1) Q: Is it a mandatory to have a power of 2 keys (reads) in my SAM files?
   ...
   A: No it is not a mandatory. It is best case but in practice and most of the time it is impossible (you can think of the FFTW tool). The sorting ask for a power of 2 keys to work so internally we fill the coordinates vector to fit that power of 2. You may think this will occur an important overhead, but in fact not to much, around 10%. Why? Because the algorithm works in (log(N))² time complexity. So whether the number of keys is between 2^N and 2^(N+1) the lower and upper bounds are set and your sorting time is predictable (or real time). In conclusion each time you increase the keys with a factor of 2 you increase the time by a factor of (log(2))².  
